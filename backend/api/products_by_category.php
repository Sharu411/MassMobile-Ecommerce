...
$sql = "
SELECT p.*, d.warrenty AS warranty
FROM products p
...
while ($row = $result->fetch_assoc()) {
    ...
    if (empty($warranty_raw) || strtoupper(trim($warranty_raw)) === 'NILL' || strtotime($warranty_raw) === false) {
        $row['warranty'] = 'NILL';
    } else {
    ...
    // Review summary
    $reviewQuery = $conn->prepare("SELECT ROUND(AVG(rating), 1) as avg_rating, COUNT(*) as total_reviews FROM product_reviews WHERE product_id = ?");
